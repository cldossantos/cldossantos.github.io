...
Let's compute first $\hat{\mu}$. This is an easy one:

```{r, echo=TRUE}
mu_hat <- mean(seed_trial$yield)
mu_hat
```

Next, let's compute the treatment means and extract $\hat{\tau}_i$.
...
taus$mu_hat <- mu_hat
taus$tau_hat <- taus$trt_means - taus$mu_hat
taus
```

Ok! We are much closer to computing all effects, we just need the errors ($\epsilon$) now. To do that, let's merge the dataset that contains the two estimated effects we've computed so far ($\hat{\tau}_i, \hat{\mu}$), with the dataset containing all observations, as the errors ($\epsilon_i$) are computed for every observation.

```{r, echo=TRUE}
seed_trial_eff <- merge(seed_trial, taus)
seed_trial_eff$epsilon <- with(seed_trial_eff, yield - mu_hat - tau_hat)
head(seed_trial_eff)
```
...
ggplot(data = seed_trial_eff)+
  geom_vline(aes(xintercept = mu_hat, col = 'mu_hat'), linetype = 1)+
  geom_segment(aes(x = mu_hat, xend = trt_means, y = rep, yend = rep, col = 'tau_hat'), linetype = 1)+
...
```{r, echo=TRUE}
SS_treatment <- sum(seed_trial_eff$tau_hat ^ 2) ## Treatment
SS_error <- sum(seed_trial_eff$epsilon ^ 2) ## Error
SS_total <- sum((seed_trial_eff$yield - seed_trial_eff$mu_hat)^2) ## Total
