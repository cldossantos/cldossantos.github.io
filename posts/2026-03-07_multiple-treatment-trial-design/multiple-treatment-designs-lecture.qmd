...
Great! This works because in this example, the variability is really random, meaning that our errors ($\varepsilon_{ij}$) are really independent from each other and normally distributed.
...
One important thing that we can think about is how, by accounting for soil variability using a blocked structure, we explicitly remove the soil variability from the residuals ($\varepsilon_{ij}$). Our linear additive model now includes a block effect, as we can see below:
