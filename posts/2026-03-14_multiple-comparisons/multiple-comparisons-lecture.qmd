...
$$Y_{ij} = \mu + b_i + \tau_j + \varepsilon_{ij}$$ $$b_i \sim N(0, \sigma^2_{block})$$
...
-   $\tau_j$ is the fungicide treatment effect
...
Our analysis of variance tells us that there is strong evidence that at least one treatment effect ($\tau_j$) is different from zero. So, next, we will proceed with what's called a means-separation test.
...
$$\alpha_E \approx 1 - (0.95)^{15} \approx 0.54$$

That is a 54% chance of at least one false positive across the experiment. Conversely, if we fix $\alpha_E = 0.05$ for the same 15 comparisons, the per-comparison error drops to roughly $\alpha_C \approx 0.003$, making it very difficult to detect modest but real differences.
...
$$
\text{HSD} = q_{\alpha, K, df} \times SE_{\bar{Y}}
$$

where $SE_{\bar{Y}} = \sqrt{MSE/n}$ is the standard error of a single treatment mean (note: this differs from the $SE$ of a difference used in the LSD formula above by a factor of $\sqrt{2}$).
...
$$LSD_{\text{bonferroni}} = t_{\frac{0.05}{2\times 15},20} \times 5.44 = t_{0.0017,20} \times 5.44$$ $$LSD_{\text{bonferroni}} = 3.33 \times 5.44 = 18.12$$
